...
#SBATCH --no-requeue
#SBATCH --exclude=cn-g001,cn-g011,cn-g008,cn-g007,cn-g012,cn-g026,cn-g025,cn-g015,cn-g017,cn-g024,cn-d003

# Job script for ManiSkill StackCube PPO fine-tuning with OpenVLA on a single A100L
# Adapt model paths in examples/embodiment/config/maniskill_ppo_openvla_stackcube.yaml before running
# Submit with: sbatch job-maniskill-stackcube-ppo.sh
...
(
  while true; do
    sleep 1800
    ls -dt ~/projects/libero_rl/RLinf/logs/*/maniskill_ppo_openvla_stackcube/checkpoints/global_step_* 2>/dev/null | tail -n +3 | xargs rm -rf
  done
) &
CLEANUP_PID=$!
...
exec srun bash examples/embodiment/run_embodiment.sh maniskill_ppo_openvla_stackcube
